...
class UpdatePayment extends Component {
	...
	componentDidMount() {
		getPayment(this.props.match.params.id).then((res) => {
			this.setState({
				...
				payoutDate: res.data.payoutDate,
				statusId: res.data.statusId,
				saveId: res.data.saveId
			});
	...
	handleUpdatePaymnet = () => {
		const { Id, amount, paymentDate, payoutDate, statusId, saveId } =
			this.state;
		...
		return (
			<>
				...
				<Grid container style={{ textAlign: "center", marginTop: 15 }}>
					<Grid item xs={12}>
						<Typography variant="h5">Aktualizacja opłaty</Typography>
					</Grid>
									...
									<Grid item xs={12}>
										<TextField
											...
											value={
												...
												this.state.paymentDate.substring(0, 10)
											}
											style={{ width: 225 }}
											InputLabelProps={{
												shrink: true
									...
									<Grid item xs={12}>
										<TextField
											...
											value={
												...
												this.state.payoutDate.substring(0, 10)
											}
											style={{ width: 225 }}
											InputLabelProps={{
												shrink: true
					...
					<Grid item xs={12} style={{ marginTop: 20 }}>
						<Button
							variant="contained"
							color="primary"
							onClick={this.handleUpdatePaymnet}
						>
							Aktualizuj
						</Button>
					</Grid>
